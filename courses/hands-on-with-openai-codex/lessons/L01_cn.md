...
type: course
title: "第 1 课：在 Codex 线程里复现故障"
summary: "把文件夹作为 Codex 项目打开，开启第一个线程，用 verify.py 让失败变得客观——在应用内完成，而不是在应用旁边。"
lang: zh-CN
content_version: 2
status: reviewed
...
hints:
  - title: "本检查点考察什么"
    body: "先复现失败再动手修：`python verify.py starter --expect-failure` 必须打印出点名的失败测试。在 Codex 应用里，线程是干活的地方——但命令仍然是证据。"
  - title: "线程是任务，不是聊天"
...
# 第 1 课：在 Codex 线程里复现故障

## 目标

本课结束时，你能在 Codex 桌面应用里打开课程文件夹、运行课程的客观
验证命令，并解释「starter 失败」在测试名里的含义。本课不改任何代码。
...
在 Codex 应用里把课程文件夹作为项目打开，然后开一个新线程。线程是
单个任务：它装着你的提示、Agent 的工具调用和产生的 diff。对它说：

**“读 COURSE.md——它是本文件夹的教学契约。再读 TASK.md 和
starter/report_tool.py。不要改任何文件。告诉我测试期望哪些行为、
starter 缺了哪些。”**

趁它读的时候，自己跑客观验证——在应用的终端里，或者让 Agent 在线程里
跑（命令执行会请求你的审批；批准前先读命令）：
...
两条命令都以 0 退出——但含义相反。第一条证明 starter *确实处于正确的
未完成态*；第二条证明目标可达。这一对命令是整门课的完成证据。
...
3. 你在练习里找到的那一行会让 starter 崩溃、被隔离，还是悄悄通过？

## 预期证据

两条命令的完整输出记录，加你的三个答案。保留好——第 4 课还会用到。
