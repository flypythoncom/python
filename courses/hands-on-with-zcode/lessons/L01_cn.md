...
id: course-zcode-l01
type: course
title: "第 1 课：在工作区里复现故障"
summary: "把文件夹作为 ZCode 工作区打开，给 Agent 第一个任务，用 verify.py 让失败变得客观——在应用内完成，而不是在应用旁边。"
lang: zh-CN
content_version: 1
status: reviewed
...
hints:
  - title: "本检查点考察什么"
    body: "先复现失败再动手修：`python verify.py starter --expect-failure` 必须打印出点名的失败测试。你在工作区终端跑也好、Agent 作为任务步骤跑也好，命令就是证据。"
  - title: "工作区，不是聊天框"
...
本课结束时，你把课程文件夹作为 ZCode 工作区打开，运行过课程的
客观验证命令，并能解释「starter 失败」在测试名里的含义。本课
不改任何代码。
...
把课程文件夹作为 ZCode 工作区打开，然后给 Agent 第一个任务——
直接引用文件：
...
趁它读的时候，跑客观验证——在工作区终端里，或让 Agent 作为任务
步骤跑：
