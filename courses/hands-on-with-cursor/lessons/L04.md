...
hints:
  ...
  - title: "Review the delta, not the vibes"
    body: "Git's working-tree diff shows the whole accumulated change. Read it as a reviewer: every hunk maps to a contract line, nothing extra moved, no test weakened."
...
1. **Walk a skin end-to-end.** Pick a `scenario/<skin>/` folder and run
   ...
   by hand.
2. **Review the accumulated diff.** `git diff` (or Cursor's source-control
