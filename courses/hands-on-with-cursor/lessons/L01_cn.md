...
type: course
title: "第 1 课：在 Cursor 里复现故障"
summary: "把文件夹作为 Cursor 项目打开，让 Agent 模式读教学契约，用 verify.py 让失败变得客观——在编辑器内完成，而不是在编辑器旁边。"
lang: zh-CN
content_version: 1
status: reviewed
...
hints:
  - title: "本检查点考察什么"
    body: "先复现失败再动手修：`python verify.py starter --expect-failure` 必须打印出点名的失败测试。在 Cursor 里，Agent 模式是干活的地方——但命令仍然是证据。"
...
# 第 1 课：在 Cursor 里复现故障

## 目标

本课结束时，你能在 Cursor 里打开课程文件夹、运行课程的客观验证
命令，并解释「starter 失败」在测试名里的含义。本课不改任何代码。
...
在 Cursor 里打开课程文件夹（`File → Open Folder`），开一个对话并
确认处于 **Agent** 模式——这个模式才能读文件、跑命令。对它说：

**“读 COURSE.md——它是本文件夹的教学契约。再读 TASK.md 和
starter/report_tool.py。不要改任何文件。告诉我测试期望哪些行为、
starter 缺了哪些。”**

趁它读的时候，自己在集成终端里跑客观验证：
