...
- [Agent 工具基础](foundation/)——驱动一门编码 Agent（Claude Code、
  Codex、Cursor、DeepSeek Harness、Kimi Code、ZCode 六选一），写出单一
  真源规则文件，并亲手验证 Agent 的产出。模块：六选一工具课、规则与
  验证两门课，以及一个双 Agent 路线挑战（200 分）。入门总览模块为
  可选加分项（10 分）——不计入徽章。
